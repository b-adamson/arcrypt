...
function solToLamportsBN(sol: string): BN {
  ...
  const lamports = BigInt(whole || "0") * 1_000_000_000n + BigInt(fracPadded);
...
function txFromIxs(...ixs: TransactionInstruction[]): Transaction {
...
function auctionTypeToArg(auctionType: AuctionType) {
  if (auctionType === "FirstPrice") return { firstPrice: {} };
  if (auctionType === "Vickrey") return { vickrey: {} };
  if (auctionType === "Uniform") return { uniform: {} };

  throw new Error("Invalid auction type");
}

function assetKindToArg(assetKind: AssetKind) {
...
async function buildAuctionCore(params: CreateAuctionParams): Promise<AuctionCoreResult> {
  const {
    programClient,
    programId,
    publicKey,
  ...
  const ctx =
    ...
    (() => {
      ...
      const [auctionPda] = PublicKey.findProgramAddressSync(
        [Buffer.from("auction"), authority.toBuffer(), randSeed],
        programId
      );
      ...
      } satisfies MakeAuctionServerResponse;
    })();
  ...
  if (assetKind === "MetadataOnly") {
    const auctionIx = await programClient.methods
      .createMetadataAuction(
        computationOffset,
        Buffer.from(ctx.auctionSeedHex, "hex"),
        auctionTypeArg,
  ...
  const vaultAuthorityPk = PublicKey.findProgramAddressSync(
    [Buffer.from("vault-authority"), auctionPda.toBuffer()],
    programId
  )[0];
  ...
  const auctionIx = await programClient.methods
    .createTokenAuction(
      ...
      new BN(minBidLamports.toString()),
      new BN(endTime),
      new BN(saleAmountBaseUnits.toString()),
      metadataUri
    )
    .accounts({
      ...commonAccounts,
...
Check it out here: http://localhost:3000/bid?auctionPk=${core.srv.auctionPda}`;
  ...
  const instructionData = [core.auctionIx].map(
    (ix) =>
      new InstructionData({
...
// =======================================================
// Auction action SDK
// Place bid, determine winner, reclaim, refund, settle
// =======================================================
...
function txFromInstruction(ix: TransactionInstruction): Transaction {
  return new Transaction().add(ix);
}

function txBundleFromInstruction(ix: TransactionInstruction): AuctionActionBundle {
  return {
...
function deriveWinnerEscrowPda(auctionPk: PublicKey, winnerPk: PublicKey, programId: PublicKey) {
  return PublicKey.findProgramAddressSync(
    [Buffer.from("escrow"), auctionPk.toBuffer(), winnerPk.toBuffer()],
    programId
  )[0];
}

function auctionStatusKey(auction: any): string {
  return enumKey(auction?.status ?? auction?.statusKey ?? auction?.status_key).toLowerCase();
}

function auctionTypeKey(auction: any): string {
...
function auctionResolvedWinnerKeys(auction: any): string[] {
  ...
  return winners
    ...
    .filter(Boolean)
    .filter((w: string) => w !== DEFAULT_PUBKEY);
}

function auctionIsWinnerClaimed(auction: any, walletBase58: string): boolean {
  const type = auctionTypeKey(auction);
...
function auctionIsMetadataOnly(auction: any): boolean {
  return auctionAssetKindKey(auction) === "metadataonly";
}

function auctionIsTokenAuction(auction: any): boolean {
  const kind = auctionAssetKindKey(auction);
  return kind === "fungible" || kind === "nft";
}

function resolveWinnerTargetForSettlement(auction: any, walletBase58: string): string | null {
  const type = auctionTypeKey(auction);
  const winners = auctionResolvedWinnerKeys(auction);
  ...
  if (Array.isArray(paidMulti)) {
    const nextIdx = paidMulti.findIndex((paid: boolean) => !paid);
    if (nextIdx >= 0 && winners[nextIdx]) return winners[nextIdx];
  }
...
export async function buildPlaceBidTransaction(
  ...
  const computationOffset = new BN(crypto.randomBytes(8), "hex");
  const computationPk = getComputationAccAddress(
  ...
  const bidderLo = BigInt(
    new BN(Buffer.from(bidderBytes.slice(0, 16)), "le").toString()
  );
  const bidderHi = BigInt(
    new BN(Buffer.from(bidderBytes.slice(16, 32)), "le").toString()
  );
  ...
  const ciphertext = cipher.encrypt(
    [bidderLo, bidderHi, amount, price],
    nonce
  );
  ...
  // ========================
  const bidderWsolAta = getAssociatedTokenAddressSync(
    NATIVE_MINT,
    publicKey
  );
...
export async function buildDetermineWinnerTransaction(
  ...
  if (!programClient || !publicKey) throw new Error("Wallet / program client not ready.");

  const arciumProgramIdStr = process.env.NEXT_PUBLIC_ARCIUM_PROGRAM_ID;
  if (!arciumProgramIdStr) throw new Error("Missing NEXT_PUBLIC_ARCIUM_PROGRAM_ID in env");
...
export async function buildReclaimUnsoldTransaction(
  ...
  if (!creatorPk.equals(publicKey)) throw new Error("Only the creator can reclaim an unsold item.");
...
export async function buildSettleWinnerTransaction(
  ...
  if (!targetWinner) throw new Error("Could not determine settlement target.");

  const targetWinnerPk = new PublicKey(targetWinner);

  const winnerEscrowPda = deriveWinnerEscrowPda(
    auctionPk,
    targetWinnerPk,
    programId
  );
  ...
  // TOKEN / NFT
  // ========================
  const prizeMintPk = new PublicKey(
    auctionData.tokenMint ?? auctionData.token_mint
  );

  const prizeVaultPk = new PublicKey(
...
export async function createPlaceBid(
  ...
  if (!mxePublicKey) throw new Error("MXE key missing");
  ...
  const escrowPda = PublicKey.findProgramAddressSync(
    [Buffer.from("escrow"), auctionPk.toBuffer(), publicKey.toBuffer()],
    programId
  )[0];
...
export async function createDetermineWinner(params: {
  ...
  const ix = await (programClient.methods as any)[methodName](computationOffset)
    .accounts({
      settler: publicKey,
      auction: auctionPk,
      signPdaAccount: signPda,
