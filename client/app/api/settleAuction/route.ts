...
export async function POST(req: Request) {
  try {
    ...
    const program = await createReadOnlyProgram(
      rpcUrl,
      programIdStr
    );
    ...
    const auctionData =
      await program.account.auction.fetch(auctionPkObj);

    const auctionType = enumKey(
      auctionData.auctionType
    ).toLowerCase();

    const status = enumKey(
      auctionData.status
    ).toLowerCase();

    if (status !== "resolved") {
      return NextResponse.json(
        { error: "Auction is not resolved yet" },
        { status: 400 }
      );
    }
    ...
    const winners: string[] =
      auctionType === "uniform"
        ? (auctionData.winners ?? [])
            ...
            .filter(Boolean)
        : (() => {
            const w = auctionData.winner;
            return w ? [new PublicKey(w).toBase58()] : [];
          })();

    if (winners.length === 0) {
      return NextResponse.json(
        { error: "No resolved winners found" },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < winners.length; i++) {
      if (auctionType === "uniform" && isPaid(auctionData, i)) {
        continue;
      }
      ...
      const bundle =
        await buildSettleWinnerTransaction({
