      ...
      <article className="grid min-h-[260px] gap-0 lg:grid-cols-[320px_1fr]">
        <div className="relative min-h-[260px] border-b border-[var(--line)] lg:border-b-0 lg:border-r">
          {item.image ? (
            ...
            <div className="flex h-full min-h-[260px] items-center justify-center bg-[var(--background)] text-sm text-[var(--muted)]">
              No image
            </div>
          )}
        </div>
        ...
        >
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {!isMetadataOnly && (
              <Field
                label="Token mint"
                value={item.tokenMint ? shorten(item.tokenMint) : "Unavailable"}
              />
            )}
            <Field
              label="Amount"
              value={isMetadataOnly ? "Metadata only" : item.saleAmount || "0"}
            />
            <Field
              label="Auction PK"
              value={shorten(item.auctionPk)}
              copyValue={item.auctionPk}
            />
            <Field
              label="Metadata URI"
